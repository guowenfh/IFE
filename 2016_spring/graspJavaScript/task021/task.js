(function() {
    ...
    function removeList(obj, arr) {
        gg.addEvent(obj, 'mouseover', function(ev) {
        ...
        gg.addEvent(obj, 'mouseout', function(ev) {
            if (ev.target.nodeName.toLowerCase() === 'li') {
                var target = ev.target;
        ...
        if (arr) {
            gg.addEvent(obj, 'mousedown', function(ev) {
                if (ev.target.nodeName.toLowerCase() === 'li') {
                    var target = ev.target;
    ...
    function init() {
        ...
        gg.addEvent(tagInp, 'keydown', function(ev) {
            if (ev.keyCode === 13 || ev.keyCode === 186 || ev.keyCode === 188 || ev.keyCode === 32) {
                tagInpFun(gg.trim(tagInp.value), tagList);
                this.value = '';
            }
        });
        removeList(tagList, arr);
        gg.addEvent(interestBtn, 'click', function() {
            interestInpFun(gg.trim(interestInp.value), interestList);
        });
    }
    init();
})();
